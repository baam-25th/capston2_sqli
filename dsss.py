                ...
                for prefix, boolean, suffix, inline_comment in itertools.product(PREFIXES, BOOLEAN_TESTS, SUFFIXES, (False, True)):
                    if not vulnerable:
                        ...
                        if vulnerable:
                            json_form = {
                                "type":"SQLi",
                                "url": payloads[True].split("=")[0]+"=",
                                "payload": unquote(payloads[True].split("=")[1][1:]),
                                "Review": payloads[True].split("=")[0]+"=" + payloads[True].split("=")[1][1:]
                            }
...
def readAndGetUrls(path):
    lastStorageUrls = []
    with open(path, "r") as rfile:
        storageUrls = rfile.readlines()
    for temp in storageUrls:
        lastStorageUrls.append(temp.strip("\n"))


    return lastStorageUrls
...
if __name__ == "__main__":
    ...
    if path.exists("urls.txt"):
        testUrls = readAndGetUrls("urls.txt")
        for eachUrl in testUrls:
            result = scan_page(eachUrl if eachUrl.startswith("http") else "http://%s" % eachUrl)
            print("scan results: %s vulnerabilities found\n" % ("possible" if result else "no"))
    else:
